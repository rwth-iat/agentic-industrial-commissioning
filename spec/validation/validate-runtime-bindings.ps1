...
function Test-RuntimeBindingSemantics {
    ...
    for ($bindingIndex = 0; $bindingIndex -lt $bindings.Count; $bindingIndex++) {
        ...
        $writeSemantics = Get-PropertyValue -Object $binding -Name 'write_semantics'
        $writeSemanticsType = [string](Get-PropertyValue -Object $writeSemantics -Name 'type')
        $writeConstraints = Get-PropertyValue -Object $binding -Name 'write_constraints'
        if ($access -eq 'read' -and $null -ne $writeSemantics) {
            Add-ValidationError -Errors $Errors -Path "${bindingPath}.write_semantics" -Message "read-only binding '${bindingId}' must not declare write semantics"
        }
        if ($writeSemanticsType -eq 'pulse' -and $datatype -ne 'BOOL') {
            Add-ValidationError -Errors $Errors -Path "${bindingPath}.write_semantics" -Message "pulse binding '${bindingId}' requires BOOL datatype"
        }
