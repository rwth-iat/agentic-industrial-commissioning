...
if ($ConfigPath) {
    ...
    $remoteConfig = if ($localConfig.ContainsKey('Remote')) {
        $localConfig.Remote
    }
    else {
        $localConfig
    ...
    if ($remoteConfig -isnot [System.Collections.IDictionary]) {
        throw "Remote configuration in '$ConfigPath' must be a data-file hashtable."
    }
    if (-not $PSBoundParameters.ContainsKey('ComputerName') -and $remoteConfig.ContainsKey('ComputerName')) {
        $ComputerName = [string]$remoteConfig.ComputerName
    }
    if (-not $PSBoundParameters.ContainsKey('Authentication') -and $remoteConfig.ContainsKey('Authentication')) {
        $Authentication = [string]$remoteConfig.Authentication
    }
}
...
if ($Authentication -notin @('Default', 'Negotiate', 'Kerberos')) {
...
try {
    Write-Host 'TwinCAT Remote Bridge'
    ...
    if ($null -eq $credentialInUse) {
        $credentialInUse = Get-Credential -Message 'Authenticate for the configured engineering host'
    }
    ...
    Write-Host 'Stopping TwinCAT Remote Bridge...'
    if ($null -ne $listener) {
